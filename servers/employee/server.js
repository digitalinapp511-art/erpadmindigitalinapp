...
loadRootEnv(root);
if (process.env.EMPLOYEE_SERVICE_PORT) {
  process.env.BACKEND_PORT = String(parseInt(process.env.EMPLOYEE_SERVICE_PORT, 10));
}
...
const employeeRoutes = require(path.join(root, 'portals/employee/app'));

const app = createServiceApp();
const port = parseInt(process.env.EMPLOYEE_SERVICE_PORT || process.env.BACKEND_PORT, 10);
...
(async () => {
  ...
  app.listen(port, '0.0.0.0', () => {
    console.log(`Employee portal service → ${getBackendUrl()} (port ${port})`);
